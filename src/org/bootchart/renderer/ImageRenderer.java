...
/**
 * ImageRenderer renders the boot chart as a Java 2D <code>BufferedImage</code>.
 * Subclasses may then encode the image in different formats (e.g. PNG or EPS).
 */
public abstract class ImageRenderer extends Renderer {
    ...
    /** Process label font. */
    private static final Font PROC_TEXT_FONT = new Font(fontFamily, Font.PLAIN,
            12);
    ...
    private static final Stroke DEP_STROKE = new BasicStroke(1.0f,
            BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 10.0f, new float[] {
                    2.0f, 2.0f }, 0.0f);

    /** Boot duration time format. */
    private static final DateFormat BOOT_TIME_FORMAT = new SimpleDateFormat(
            "m:ss", Common.LOCALE);
    ...
    /**
     * Whether to allow usage of transparency. Certain renderers (e.g. EPS) will
     * produce better results if the colors aren't transparent.
     */
     ...
     * 
     * @param headers
     *            header properties to include in the title banner
     * @param bootStats
     *            boot statistics
     * @param os
    ...
    public void render(Properties headers, BootStats bootStats, OutputStream os)
        ...
        if (g == null) {
            ...
            int type = allowAlpha ? BufferedImage.TYPE_INT_ARGB
        ...
        if (cpuStats != null) {
            ...
            for (Iterator i = cpuStats.getSamples().iterator(); i.hasNext();) {
                ...
                int posX = rectX
                        + (int) ((sample.time.getTime() - procTree.startTime
                                .getTime()) * rectW / procTree.duration);
                if (posX < rectX || posX > rectX + rectW) {
                    // log.warning("Cropped sample " + pi + ": " + sample);
                    continue;
                }
                int posY = barY
                        - (int) ((sample.user + sample.sys + sample.io) * barH);
                if (lastPoint != null) {
        ...
        if (diskStats != null) {
            ...
            for (Iterator i = diskStats.getSamples().iterator(); i.hasNext();) {
                ...
                Date endTime = new Date(procTree.startTime.getTime()
                        + procTree.duration);
            ...
            lastPoint = null;
            setColor(g, DISK_TPUT_COLOR);
            double maxTPut = ProcDiskStatParser.getMaxDiskTPut(diskStats
                    .getSamples());
            for (Iterator i = diskStats.getSamples().iterator(); i.hasNext();) {
                Sample s = (Sample) i.next();
            ...
            for (Iterator i = diskStats.getSamples().iterator(); i.hasNext();) {
                ...
                if (sample.read + sample.write == maxTPut) {
                    ...
                    String label = (int) Math
                            .round((sample.read + sample.write) / 1024.0)
                            + " MB/s";
            ...
            lastPoint = null;
            setColor(g, FILE_OPEN_COLOR);
            int maxFiles = FileOpenSample.getMaxFileOpens(diskStats
                    .getSamples());
            for (Iterator i = diskStats.getSamples().iterator(); i.hasNext();) {
                Sample s = (Sample) i.next();
        ...
        /*
         * // draw process tree lines setColor(g, TICK_COLOR); for (int i = 0; i
         * <= rectH; i += procH) { g.drawLine(rectX, rectY + i, rectX + rectW,
         * rectY + i); }
         */

        g.setFont(PROC_TEXT_FONT);
        if (procTree.processTree != null) {
            drawProcessList(procTree.processTree, -1, -1, procTree, rectY,
                    procH, new Rectangle(rectX, rectY, rectW, rectH));
        }
    ...
    private void drawHeader(Properties headers, int offX, long duration) {
        ...
        if (headers != null) {
            ...
            g.drawString(uname, offX, headerY + (hoff++)
                    * (TEXT_FONT.getSize() + 2));
            String rel = "release: "
                    + headers.getProperty("system.release", "");
            g.drawString(rel, offX, headerY + (hoff++)
                    * (TEXT_FONT.getSize() + 2));
            String cpu = "CPU: " + headers.getProperty("system.cpu", "");
            cpu = cpu.replaceFirst("model name\\s*:\\s*", "");
            g.drawString(cpu, offX, headerY + (hoff++)
                    * (TEXT_FONT.getSize() + 2));
            if (headers.getProperty("profile.process") != null) {
                String app = "application: "
                        + headers.getProperty("profile.process", "");
                g.drawString(app, offX,
                        headerY + (hoff++) * (TEXT_FONT.getSize() + 2));
            } else {
                String kopt = "kernel options: "
                        + headers.getProperty("system.kernel.options", "");
                g.drawString(kopt, offX,
                        headerY + (hoff++) * (TEXT_FONT.getSize() + 2));
            }
        }
        Date dur = new Date(Math.round(duration / 1000.0) * 1000);
        String bootTime = "time: " + BOOT_TIME_FORMAT.format(dur);
        g.drawString(bootTime, offX, headerY + (hoff++)
                * (TEXT_FONT.getSize() + 2));
    }
    ...
    private int drawProcessList(List processList, int px, int py,
        ...
        for (Iterator i = processList.iterator(); i.hasNext();) {
            ...
            /*
             * if (lastProc != null && (lastProc.startTime.getTime() +
             * lastProc.duration <= proc.startTime .getTime())) { y -= procH; }
             */
            int offY = procH;
            drawProcess(proc, px, py, procTree, y, procH, rect);
            int px2 = rect.x
                    + (int) ((proc.startTime.getTime() - procTree.startTime
                            .getTime()) * rect.width / procTree.duration);
            int py2 = y + procH;
            y = drawProcessList(proc.childList, px2, py2, procTree, y + offY,
                    procH, rect);
            // lastProc = proc;
        }
        return y;
    ...
    private void drawProcess(Process proc, int px, int py,
        ...
        if (px != -1 && py != -1) {
            if (Math.abs(px - x) < 3) {
                ...
                g.drawLine(px - depOffX, y + procH / 2, px - depOffX, py
                        - depOffY);
        ...
        for (Iterator i = proc.samples.iterator(); i.hasNext();) {
            ...
            Date endTime = new Date(proc.startTime.getTime() + proc.duration
                    - procTree.samplePeriod);
            ...
            int tx = rect.x
                    + (int) Math
                            .round(((sample.time.getTime() - procTree.startTime
                                    .getTime()) * rect.width / (double) procTree.duration));
    ...
    /**
     * Sets the current color. If <code>allowAlpha</code> is not set (e.g. for
     * the EPS renderer), an opaque color is used.
     * 
     * @param g
     *            graphics context
     * @param col
    ...
    private void setColor(Graphics g, Color col, Color backCol) {
        if (!allowAlpha) {
            ...
            int red = col.getRed();
            red = red
                    + (int) ((backCol.getRed() - red)
                            * (backCol.getRed() - alpha) / 255.0);
            int green = col.getGreen();
            green = green
                    + (int) ((backCol.getGreen() - green)
                            * (backCol.getGreen() - alpha) / 255.0);
            int blue = col.getBlue();
            blue = blue
                    + (int) ((backCol.getBlue() - blue)
                            * (backCol.getBlue() - alpha) / 255.0);
     ...
     * @return <code>true</code> if the statistics instance contains any samples
     *         of the specified class, <code>false</code> otherwise
     */
    private static boolean hasSamples(Stats stats, Class sampleClass) {
        if (stats == null || stats.getSamples() == null
                || stats.getSamples().isEmpty()) {
            return false;
        }
